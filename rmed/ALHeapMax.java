...
public class ALHeapMax
{
     ...
     * 2) While the Max Heap condition is not met, swap addVal with its parent
     * 3) Stop swapping once the Max Heap condition is met
     *****************************************************/
  ...
  size--;
	...
	Integer ret = peekMax();          //get the integer that will be removed
	...
	int possNew = maxChildPos(index); //stores the index of the child that has the potential to swap
	...
	if (left < _heap.size() && right < _heap.size()) {  //check that it does have 2 children
	    ...
	    if (maxOf(a,b) == a) {                          //if a is smaller
